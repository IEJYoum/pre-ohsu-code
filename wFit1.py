...
def main(data):
    ...
    startNode = obj.Node(0,99,np.ones(shape[1]-2),startThF,[])
    shape = data.shape
    predictions = []
    answered = 0 
    while answered < shape[1]:
        ...
        tunedNode = tuneNode(startNode,X,Y,wiggleF,WIGGLESpERnODE,TUNEf)
        out1D = tunedNode.runSelf1(X)
        predictions.append(out1D)
        answered += 1
    return(predictions)
